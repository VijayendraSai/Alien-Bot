...
def bot1(num_aliens):
    ...
    while (isAlive and time < 1000):
        ...
        # Return if there is no possible path
        if path is None:
            return (crew_saved, time, isAlive)

        path.append(bot_cell)
        path.reverse()
        ...
        for i in range(1, len(path)):
            ...
            if (ship[curr_cell] == 9):
                ...
                isAlive = False
                return (crew_saved, time, isAlive)
            update_bot_position(path[i - 1], path[i])

            # Generating alien movement
            generate_alien_movements()

            # Checking for the killing condition
            if (ship[curr_cell] == 9):
                print("Bot Dies!")
                isAlive = False
                return (crew_saved, time, isAlive)
...
while nums<=50:
    ...
    for i in range(100):
        print(i)
        x, y, z = bot1(nums)
        result.append({'Bot': 1, 'NumAliens': nums, 'CrewSaved': x, 'TimeAlive': y, 'IsAlive': z})
        print(x, y, z)
        total += x
        print("**********************************************")
...
# Saving the results
df.to_excel('results-1.xlsx', index=False)