...
# Function for maintaining the valid neighbors of alien cells
...
# Function for implementation of the A* algorithm (passing key to decide between Bot-3 and 2 strategy)
# (key=0 --> Bot-3 ; Key=1 --> Bot-2)
def a_star(key=0):
    global bot_cell
    global crew_cell
    global ship

    # Checks validity based on aliens and also their neighboring cells
    def is_valid(x, y):
        return 0 <= x < len(ship) and 0 <= y < len(ship[0]) and ship[x][y] == 0 and (x,y) not in alien_neighbors

    # Fallback to Bot-2 strategy in case of no valid path
    def is_valid2(x, y):
...
while nums<=50:
    ...
    for i in range(100):
        print(i)
        x, y, z = bot3(nums)
        result.append({'Bot': 3, 'NumAliens': nums, 'CrewSaved': x, 'TimeAlive': y, 'IsAlive': z})
        print(x, y, z)
        total += x
        print("**********************************************")
...
# Saving the results
df.to_excel('results-3-20.xlsx', index=False)